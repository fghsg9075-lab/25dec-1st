...
const App: React.FC = () => {
          ...
          getUserData(state.user.id).then(cloudUser => {
             if (cloudUser) {
                 ...
                 if (currentStr !== cloudStr) {
                     ...
                     // Auto-Switch to Admin Dashboard if Role changes to ADMIN
                     if (cloudUser.role === 'ADMIN' && state.view !== 'ADMIN_DASHBOARD') {
                         setState(prev => ({...prev, user: cloudUser, view: 'ADMIN_DASHBOARD'}));
                     } else {
  ...
  useEffect(() => {
    ...
    if (state.view !== 'BOARDS') {
        window.history.pushState({ view: state.view }, '', window.location.pathname);
    }
  ...
  }, [state.view]); // Re-bind when view changes to capture correct 'goBack' context
